    ...
    if(req.body.type=="customer")
    {
        customermodel.findOne({username: us}, function(err, customer){
            if(pass == customer.password)
            {
                ...
                res.sendFile("C:/Users/admin/Desktop/projects/web-master/views/homepage.html")
            }
...
app.listen(2600, 'localhost', function(){
    console.log("Connected to server on port 2600")
})
...
<!-- 
            ...
            <p><label>Shop: </label><span><%=item.shopname%></span></p> 
          </div>
          <div class="form-group">
            <p><label>Owner: </label><%= item.owner%></p>
          </div>
          <div class="form-group">
            <p><label>itemname: </label><%= item.itemname%></p> 
          </div>
          <div class="form-group">
            <p><label>description: </label><%= item.description%></p>
          </div>
          <div class="form-group">
            <p><label>distance: </label><%=item.distance%></p> 
          </div>
